...
export class Board {
    ...
    find(inputString) {
        ...
        const checkValidity = function (x, y, prefix, visitedTrail) {
            //boundary condition
            if (x >= 0 && x < that.height && y >= 0 && y < that.width) {

                const isVisited = function (x, y, visitedArr) {
                ...
                const checkPrefixMatch = function (boardWord, inputString) {
                    const indices = [];
        ...
        const addNeighbour = function (neighbourX, neighbourY, prefix, visited) {
        ...
        const checkWordFound = function (boardWord, inputString) {
            const indices = [];
            for (let i = 0; i < boardWord.length; i++) {
                if (boardWord[i] === '*')
                    indices.push(i);
